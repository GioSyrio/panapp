...
def norm(line):
    """Strip embedded line numbers (e.g., "1   x←y" → "x←y")."""
    return re.sub(r'^\d{1,2}\s{1,4}', '', line)

def hl(line):
    line = norm(line)  # strip embedded line numbers
    line = e(line)
    line = re.sub(KW, r'<span class="kw">\1</span>', line)
    # Highlight numbers: only standalone digits, not inside tags or entities
...
def extract_q_sections(doc, qid, diagram_imgs):
    """Parse question DOCX into structured sections."""
    sections = []
    i = 0
    while i < len(doc.paragraphs):
        ...
        # Check for gap-fill markers in paragraph text
...
# ── Answer extraction → step-by-step ────────────────────────────────────────
...
def extract_answer_steps(docx_path):
    ...
    for p in doc.paragraphs:
        ...
        if m:
            steps.append({"label": m.group(1), "text": m.group(2)})
        elif steps and len(t) > 10:
            steps[-1]["text"] += " " + t
        elif len(t) > 10:
            steps.append({"label": "", "text": t})
    ...
    for table in doc.tables:
        ...
        if code_lines:
            steps.append({"label": "Code", "text": "", "code_lines": code_lines})
    return steps

def render_answer_html(ans_text, docx_path):
    steps = extract_answer_steps(docx_path)
    if not steps:
        return f'<div class="sol-text">{e(ans_text)}</div>'
    html = ''
...
def main():
    ...
    # Load previous v2 to preserve extra fields (hints, llm_solutions, etc.)
    prev_v2 = {}
    ...
    for q in questions:
        ...
        # Ensure diagram section exists if images were extracted
        ...
        # Start with previous entry if exists (preserves hints, llm_solutions)
    ...
    print(f"  Total sections: {sum(len(q['sections']) for q in output)}")

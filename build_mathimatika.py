#!/usr/bin/env python3
"""Build questions_v2.json for Mathematics (Genikis) from DOCX — answer_html + part fix + images"""
...
def main():
    ...
    p.add_argument("--subject", default="mathimatika")
    args = p.parse_args()
    ...
    dmap = {}
    if os.path.exists(MAP_FILE): dmap = json.load(open(MAP_FILE, encoding="utf-8"))
    ...
    for q in v2:
        ...
        if os.path.exists(qpath):
            ...
            if str(qid) in dmap: extracted_imgs = dmap[str(qid)].get("diagrams", [])
            else:
                extracted_imgs = extract_images(qpath, img_dir, qid)
                if extracted_imgs: dmap[str(qid)] = {"diagrams": extracted_imgs, "source": "mathimatika_docx"}; total_images += len(extracted_imgs)
        ...
        try:
            ...
            if new_part and new_part != q.get("part"): q["part"] = new_part; fixed_parts += 1
            if not q.get("question_text"):
                text = re.sub(r'<[^>]+>', ' ', q_html)
                q["question_text"] = re.sub(r'\s+', ' ', text).strip()[:3000]
        except Exception as e:
            print(f"  ⚠️  Q{qid} DOCX error: {e}")
        if os.path.exists(apath):
            try:
                a_html = build_answer_html(apath)
                if a_html: q["answer_html"] = a_html; rebuilt_a += 1
            except Exception as e:
                print(f"  ⚠️  Q{qid} answer DOCX error: {e}")
        try: q["year"] = int(q.get("year", 0))
        except: q["year"] = 2022
        if not q.get("points"):
            pts = re.findall(r'⭐ (\d+) μονάδες', q.get("question_html", ""))
            q["points"] = sum(int(p) for p in pts) if pts else 25
    with open(v2_file, "w", encoding="utf-8") as f: json.dump(v2, f, ensure_ascii=False, indent=2)
    with open(MAP_FILE, "w", encoding="utf-8") as f: json.dump(dmap, f, ensure_ascii=False, indent=2)
    parts = {}

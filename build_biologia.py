#!/usr/bin/env python3
"""Build questions_v2.json for Biology (Biologia) from DOCX — answer_html + part fix + images"""
...
def main():
    ...
    p.add_argument("--subject", default="biologia")
    args = p.parse_args()
    ...
    for q in v2:
        ...
        if os.path.exists(qpath):
            ...
            if str(qid) in dmap: extracted_imgs = dmap[str(qid)].get("diagrams", [])
                ...
                if extracted_imgs: dmap[str(qid)] = {"diagrams": extracted_imgs, "source": "biologia_docx"}; total_images += len(extracted_imgs)
